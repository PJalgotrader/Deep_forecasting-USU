...
|---|---|---|
| Use **pycaret-core 3.5.0** (sktime-org community fork, MIT) | Same `import pycaret`, same `TSForecastingExperiment` API; the only notebook change is the install line | `evidence/smoke_results.log`, `colab/colab_tests/B_*.ipynb` |
...
- [x] Step 1 - live Colab test of 3.3.2 / pycaret-core / 4.0 (`colab/colab_tests/`, `evidence/colab_results.md`); B re-run 2026-09-16 15:45 with the final install line and `compare_models()` over all 28 models (`evidence/colab_leaderboard_all_models.csv`)
...
- `colab/colab_tests/*.ipynb` are regenerated by `build_colab_tests.py` (Drive only, removed from the repo 2026-09-16); regenerating wipes the executed outputs, so do not run it casually.
